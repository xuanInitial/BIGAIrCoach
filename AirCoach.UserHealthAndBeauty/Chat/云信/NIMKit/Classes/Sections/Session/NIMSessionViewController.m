...
- (void)viewDidLoad {
    ...
    NSMutableArray *mutArr = [_sessionDatasource modelArray];
    for (NIMMessageModel *models in mutArr) {
        
        if ([models isKindOfClass:[NIMMessageModel class]]) {
            
            
            if (models.message.messageType == NIMMessageTypeImage) {
                
                NIMImageObject *object =models.message.messageObject;
                
                LRLog(@"%@",[object url]);
                
                ZLPhotoPickerBrowserPhoto *photo4 = [[ZLPhotoPickerBrowserPhoto alloc] init];
                photo4.photoURL = [NSURL URLWithString:[object url]];
                
                [self.imageArray addObject:photo4];
    ...
    for (int i= 0; i <self.imageArray.count;i++ ) {
        ZLPhotoPickerBrowserPhoto *imgModel = self.imageArray[i];
        
        UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT)];
        [img sd_setImageWithURL:imgModel.photoURL placeholderImage:imgModel.thumbImage];
...
- (void)makeUI
{
    self.navigationItem.title = [BusinessAirCoach getCoachName];
...
- (void)sendMessage:(NIMMessage *)message didCompleteWithError:(NSError *)error
{
    ...
    if (message.messageType == NIMMessageTypeImage) {
        ...
        ZLPhotoPickerBrowserPhoto *imgModel = photo4;
...
- (void)onMessageRevoked:(NIMMessage *)message
{
    ...
    for (int i = 0; i< self.imageArray.count; i++) {
        ZLPhotoPickerBrowserPhoto * photo = self.imageArray[i];
        if ([[object url] isEqualToString:photo.photoURL.absoluteString]) {
...
- (void)headerRereshing:(id)sender
{
    ...
    [self.sessionDatasource loadHistoryMessagesWithComplete:^(NSInteger index,NSArray *memssages, NSError *error) {
        ...
        if (memssages.count) {
            ...
            LRLog(@"%@",memssages);
            for (NIMMessage *message in memssages) {
                NIMMessageModel *models = message.messageObject;
                    ...
                    if (models.message.messageType == NIMMessageTypeImage) {
                        ...
                        [arr addObject:photo4];
                    }
            }
            
           //把一组元素插入到 一个数组的前面 需要设置这组元素的长度
             [self.imageArray insertObjects:arr atIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, arr.count)]];
