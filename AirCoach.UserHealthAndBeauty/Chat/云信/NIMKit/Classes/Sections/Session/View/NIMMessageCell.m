...
//    [_retryButton setImage:[UIImage nim_imageInKit:@"icon_message_cell_error"] forState:UIControlStateHighlighted];
    [_retryButton setImage:[UIImage imageNamed:@"未发送成功"] forState:UIControlStateNormal];
...
- (void)refresh{
    ...
    if ([self needShowAvatar])
    {
        ...
        if ([self.model.message.from isEqualToString:[[NIMSDK sharedSDK].loginManager currentAccount]])
        {
            [_headImageView nim_setImageWithURL:[NSURL URLWithString:[BusinessAirCoach getHeadPortrait]] placeholderImage:[UIImage imageNamed:@"用户-默认头像"]];
        }
        else
        {
#pragma mark----云信护理师头像
            //教练头像
           [_headImageView nim_setImageWithURL:[NSURL URLWithString:[BusinessAirCoach getCoachFigure]] placeholderImage:[UIImage imageNamed:@"用户-默认头像"]];
            //_headImageView.image = [UIImage imageNamed:@"训练图"];
        }
    ...
    if([self needShowNickName])
    {
        ...
        if ([self.model.message.from isEqualToString:[[NIMSDK sharedSDK].loginManager currentAccount]])
        {
            [self.nameLabel setText:[BusinessAirCoach getNickName]];
            self.nameLabel.textAlignment = 2;
        }
        else
        {
#pragma mark----云信护理师
            [self.nameLabel setText:[BusinessAirCoach getCoachName]];
            
//            [self.nameLabel setText:@"穿鞋不怕光脚的"];
...
- (void)layoutNameLabel
{
    if ([self needShowNickName]) {
        ...
        //self.cellPaddingToNick,cellPaddingToProtrait;
        CGFloat myBubbleOriginX = self.nim_width - 20 - avatarWidth - 200;
...
//                make.top.equalTo(self.contentView.mas_top).offset(otherBubbleOriginy);
//                make.height.equalTo(@(otherNickNameHeight));
//                make.width.greaterThanOrEqualTo(@(otherNickNameWidth));
//            }];
//        }
//        else
//        {
